...
class Mutt < Formula
  ...
  patch do
    url "http://www.woolridge.ca/mutt/patches/patch-1.5.6.dw.confirm-crypt-hook.1"
    sha1 "d3d40d8994702663042e876f91507c92a759a544"
  end if build.with? "confirm-crypt-hook-patch"
  ...
  def install
    ...
    if not build.with? 'mua'
      etc = ["--with-ssl=#{Formula["openssl"].opt_prefix}",
             "--with-sasl",
             "--with-gss",
             "--enable-imap",
             "--enable-smtp",
